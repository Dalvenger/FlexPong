...
#define PID com_example_banana_testdevice2_SuperpoweredUSBAudio // Java package name and class name. Don't forget to update when you copy this code.
...
#undef PID
#define PID com_example_banana_testdevice2_MainActivity // Java package name and class name. Don't forget to update when you copy this code.
...
static bool audioProcessing(void * clientdata, int __unused deviceID, float *audioIO, int numberOfSamples, int samplerate, int numInputChannels, int numOutputChannels) {
    ...
    switch (job->kind) {
        ...
        case 2: { // Latency measurement.
            ...
            if ((job->measurer->state >= 1) && (job->measurer->state <= 10)) latencyMs = job->measurer->latencyMs;
